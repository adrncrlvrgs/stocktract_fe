...
export const ModalHeader = ({ toggle, children, className = '', closeAriaLabel = 'Close' }) => (
  <div className={cn('flex justify-between items-center p-4 border-b', className)}>
    <h5 className="text-xl font-semibold">{children}</h5>
    <button onClick={toggle} className="text-gray-500 hover:text-gray-700" aria-label={closeAriaLabel}>
      &#x2715;
    </button>
  </div>
);
...
export const ModalFooter = ({ children, className = '' }) => (
  <div className={cn('flex justify-end items-center p-4 border-t', className)}>
    {children}
  </div>
);
